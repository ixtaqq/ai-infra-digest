...
### [DONE-1] Persist corroboration_count, grounding_text, effective_score to DB

**What shipped:** Added the 3 columns to `articles` and updated `insertArticles()` to write them on each pipeline run.

**Why:** These were ephemeral — computed in `generateDigest()` and lost after the run. Persisting them enables historical confidence trend analysis and makes the web layer independently queryable from the DB.

**Pros:** Unlocks historical analysis; makes the web layer independent of the pipeline's file output; enables backfill passes.

**Tradeoff accepted:** Required a migration and added schema-drift risk; migrations remain canonical.
...
**Completed in:** Phase XIII / v14.

### [DONE-2] Ground the weekly /thesis narrative in the same data /coverage shows

**What shipped:** `generateTheses()` now includes recent per-article coverage for each ticker alongside `daily_derived_metrics` aggregates.

**Why:** The weekly bull/bear narrative and per-article coverage previously pulled from unrelated data. Grounding both in the same article data makes `/thesis` traceable to `/coverage`.
...
**Tradeoff accepted:** The grounded thesis prompt is larger and has a small token-cost increase.
